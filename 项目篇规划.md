...
大家好，我是 Mokio。从今天开始，我要带大家从零开始，一步一步手搓一个自己的 Code Agent —— **MokioClaw**。

你可能用过 Claude Code、Cursor 这类 AI 编程工具，但它们像黑盒一样，你不知道里面发生了什么。那如果我们自己动手造一个呢？从最简单的 ReAct 循环开始，一路进化到多 Agent 协作、Context Engineering、Harness Engineer，最后给它套上一个酷炫的 TUI 交互层——这就是我们接下来要做的事。

每一阶段都是可运行的、可视的。你可以在终端里输入一条指令，看着 Agent 思考、规划、执行、验证，最终交付代码。
